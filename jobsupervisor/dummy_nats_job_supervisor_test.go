...
var _ = Describe("dummyNatsJobSupervisor", func() {
	...
	Describe("Status", func() {
		...
		It("returns the received status", func() {
			statusMessage := boshhandler.NewDirectorRequest("", "set_dummy_status", []byte(`{"status":"failing"}`), 0)
		...
		It("does not change the status given other messages", func() {
			statusMessage := boshhandler.NewDirectorRequest("", "some_other_message", []byte(`{"status":"failing"}`), 0)
			handler.RegisteredAdditionalFunc(statusMessage)
			Expect(dummyNats.Status()).To(Equal("running"))
		})
	})
	...
	Describe("Start", func() {
		...
		Context("When set_task_fail flag is sent in messagae", func() {
			It("raises an error", func() {
				statusMessage := boshhandler.NewDirectorRequest("", "set_task_fail", []byte(`{"status":"fail_task"}`), 0)
				handler.RegisteredAdditionalFunc(statusMessage)
				err := dummyNats.Start()
		...
		Context("when set_task_fail flag is not sent in message", func() {
			It("does not raise an error", func() {
				statusMessage := boshhandler.NewDirectorRequest("", "set_task_fail", []byte(`{"status":"something_else"}`), 0)
				handler.RegisteredAdditionalFunc(statusMessage)
				err := dummyNats.Start()
